...
const WelcomeBonus: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await authAPI.getWelcomeBonus();
      if (response.success) {
      ...
      toast.error("Failed to load welcome bonus");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-blue-50/30 via-white to-blue-50/20 min-h-screen">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Welcome Bonus History</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              No welcome bonus records found
            </div>
          ) : (
                ...
                >
                  <div>
                    <p className="font-medium">
                      {record.description || "Welcome Bonus"}
                    </p>
