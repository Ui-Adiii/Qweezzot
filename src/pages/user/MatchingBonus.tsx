...
const MatchingBonus: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await authAPI.getMatchingBonus();
      if (response.success) {
        setData(response.data || []);
        setTotal(response.total || 0);
      }
    } catch (error: any) {
      console.error("Failed to load matching bonus:", error);
      toast.error("Failed to load matching bonus");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-sky-50/30 via-white to-sky-50/20 min-h-screen">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Matching Bonus History</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
                ...
                >
                  <div>
                    <p className="font-medium">
                      {record.description || "Matching Bonus"}
                    </p>
