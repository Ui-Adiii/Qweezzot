...
const Transactions: React.FC = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'completed': return 'bg-emerald-100/70 text-emerald-800 ring-1 ring-emerald-300/30';
      case 'processing': return 'bg-blue-100/70 text-blue-800 ring-1 ring-blue-300/30';
      case 'pending': return 'bg-amber-100/70 text-amber-800 ring-1 ring-amber-300/30';
      case 'failed': return 'bg-red-100/70 text-red-800 ring-1 ring-red-300/30';
    ...
    <div className="p-6 space-y-6 bg-gradient-to-br from-emerald-50/30 via-white to-emerald-50/20 min-h-screen">
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-emerald-700 bg-clip-text text-transparent">Transactions</h1>
        </div>
      ...
      >
        <Card className="border-emerald-200/50 bg-white/70 backdrop-blur-xl shadow-lg ring-1 ring-amber-400/10">
          <CardHeader>
            <CardTitle className="text-emerald-800">All Transactions ({filteredTransactions.length})</CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
                  ...
                  <Card key={i} className="border-emerald-200/50 bg-white/60 backdrop-blur-sm animate-pulse">
                    <CardContent className="p-4">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center space-x-3">
                          <div className="w-10 h-10 bg-emerald-200/50 rounded-lg"></div>
                          <div className="space-y-2">
                            <div className="h-4 bg-emerald-200/50 rounded w-32"></div>
                            <div className="h-3 bg-emerald-200/50 rounded w-48"></div>
                          </div>
                        </div>
                      ...
                      <CardContent className="p-4">
                        <div className="flex justify-between items-center">
                          <div className="flex items-center space-x-3">
                            <div className={`p-2 rounded-lg backdrop-blur-sm ${
                              transaction.type === 'credit'
                                ? 'bg-emerald-100/70 ring-1 ring-emerald-300/30'
                            ...
                            <div>
                              <h3 className="font-semibold text-emerald-800">{transaction.description}</h3>
                          ...
                          <div className="text-right space-y-2">
                            ...
                            <Badge className={`backdrop-blur-sm ${getStatusColor(transaction.status)}`}>
