...
import { List, DollarSign, RefreshCw } from "lucide-react";
...
const AdditionalBonus: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await authAPI.getAdditionalBonus();
      if (response.success) {
        setData(response.data || []);
        setTotal(response.total || 0);
      }
    } catch (error: any) {
      console.error("Failed to load additional bonus:", error);
      toast.error("Failed to load additional bonus");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6 bg-gradient-to-br from-blue-50/30 via-white to-blue-50/20 min-h-screen">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Additional Bonus History</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              No additional bonus records found
            </div>
          ) : (
                ...
                >
                  <div>
                    <p className="font-medium">
                      {record.description || "Additional Bonus"}
                    </p>
