...
const DirectTeam: React.FC = () => {
  ...
  const [teamData, setTeamData] = useState<DirectMember[]>([]);
  ...
  const fetchDirectTeam = async () => {
    try {
      setLoading(true);
      const response = await authAPI.getDirectTeam();
      if (response.success && response.data) {
        setTeamData(response.data);
        setStats(response.stats);
      }
    } catch (error: any) {
      console.error("Failed to load direct team:", error);
      toast.error("Failed to load direct team data");
    } finally {
      setLoading(false);
    }
  };
