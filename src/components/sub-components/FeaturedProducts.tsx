      ...
      <div className="container mx-auto px-4 relative z-10">
          ...
          {benefits.map((benefit, index) => {
            ...
            return (
              <motion.div
                ...
                className="flex items-center gap-3 bg-white/80 backdrop-blur-sm rounded-2xl p-4 shadow-soft border border-emerald-200/50 ring-1 ring-amber-400/10"
              >
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-100/70 to-amber-100/50 text-emerald-600 ring-1 ring-amber-300/20">
                ...
                {featuredProducts.map((product, index) => {
                  ...
                  return (
                    <CarouselItem key={product._id} className="pl-2 md:pl-4 basis-full sm:basis-1/2 lg:basis-1/3 xl:basis-1/4">
                      ...
                      >
                        <Card
                          className="group relative bg-white/90 backdrop-blur-sm border border-emerald-200/50 rounded-2xl shadow-soft hover:shadow-xl transition-all duration-500 hover:-translate-y-2 overflow-hidden h-full flex flex-col ring-1 ring-amber-400/10"
                          onClick={() => navigate(`/products/${product._id}`)}
                        >
                          {/* Product Badge */}
                          <div className="absolute top-4 left-4 z-20">
                            <Badge className={`bg-gradient-to-r ${badgeInfo.color} text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg ring-1 ring-white/30`}>
                              {badgeInfo.badge}
                            </Badge>
                          </div>
                          ...
                          <CardContent className="pt-6 pb-4 flex-1 flex flex-col">
                            ...
                            {/* Product Name */}
                            <h3 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-emerald-700 transition-colors duration-300 line-clamp-2 min-h-[3.5rem]">
                              {product.name}
                            </h3>
                            ...
                            {product.features && product.features.length > 0 && (
                              <div className="flex flex-wrap gap-1.5 mb-4">
                                {product.features.slice(0, 2).map((feature, idx) => (
                                  <Badge
                                    key={idx}
                                    className="text-xs bg-emerald-100/70 text-emerald-700 border border-emerald-200/50 ring-1 ring-amber-400/10 backdrop-blur-sm"
                                  >
                                ...
                                {product.features.length > 2 && (
                                  <Badge className="text-xs bg-emerald-100/70 text-emerald-700 border border-emerald-200/50 ring-1 ring-amber-400/10 backdrop-blur-sm">
                                    +{product.features.length - 2}
                                  </Badge>
                                )}
                              </div>
                            ...
                            <div className="flex items-center justify-between mt-auto">
                              ...
                              >
                                <Button 
                                  size="sm" 
                                  className="rounded-full bg-gradient-to-r from-emerald-500 to-amber-500 hover:from-emerald-600 hover:to-amber-600 text-white shadow-lg hover:shadow-emerald-500/25 transition-all duration-300 border-0 ring-1 ring-amber-300/30"
        ...
        <div className="text-center mt-12">
          ...
          >
            <Button 
              size="lg" 
              className="rounded-full px-8 bg-gradient-to-r from-emerald-500 to-amber-500 hover:from-emerald-600 hover:to-amber-600 shadow-lg hover:shadow-emerald-500/25 transition-all duration-300 text-white border-0 ring-1 ring-amber-300/30"
